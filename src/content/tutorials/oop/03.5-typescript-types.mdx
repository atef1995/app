...
Hey there! Ready to dive into what makes TypeScript absolutely incredible? 🚀 
...
## Wait, Why Should I Care About Types? 🤔
...
function greetUser(user) {
  const result = `Hello, ${user.name}!`;
  console.log(result); // Shows: "Hello, undefined!"
  return result;
}

greetUser("John"); // No error, but outputs: "Hello, undefined!"
```
...
greetUser("John"); // ❌ TypeScript catches this IMMEDIATELY
...
// 🔢 Numbers - integers, decimals, even negative numbers
...
// 📝 Strings - any text you can imagine
...
// 🎯 Array of numbers - notice the number[] syntax
...
// 📚 Array of strings - perfect for lists of names, titles, etc.
...
// 🎭 Mixed types? Use union types (we'll cover this more later!)
...
{
  // 🎯 Simple object typing - define the shape of your data
  let user: {
    name: string;
    age: number;
    isActive: boolean;
...
// 🏗️ But typing every object like this gets tedious...
...
// 🔒 Readonly properties - data that should never change
type Config = {
...
// appConfig.version = "2.0.0"; // ❌ Error: Cannot assign to 'version'
}`}
</InteractiveCodeBlock>
...
// 🎯 Basic function typing - input types and return type
...
console.log("4 × 7 =", multiplyNumbers(4, 7));

// 🎨 Optional parameters - some inputs aren't always needed
...
// 🔄 Functions that take other functions - this gets exciting!
...
// 🎪 Rest parameters - when you need flexibility
...
// 🤝 Basic union types - combining multiple types
...
// 🎭 Smart type checking - TypeScript knows which type you're using!
...
// 🎯 Literal types - super specific values only
...
function applyTheme(theme: Theme): string {
  switch (theme) {
    case "light":
      return "☀️ Light theme activated";
    case "dark":
...
// 🏗️ Discriminated unions - the power combo!
type LoadingState = {
  status: "loading";
  message: string;
};
...
function handleApiResponse(state: ApiState): string {
  ...
  switch (state.status) {
    ...
    case "success":
      return \`✅ Got \${state.count} items: \${state.data.join(", ")}\`;
...
// 🏗️ Generic classes - reusable data containers
class DataStore<T> {
  private items: T[] = [];
...
// 🔗 Generic constraints - "T must be this kind of thing"
...
// 🏗️ Let's start with a base type
type User = {
  id: number;
  name: string;
  email: string;
...
// 🎯 Partial<T> - makes all properties optional (perfect for updates!)
...
<InteractiveCodeBlock editable={true} language="typescript">
{`// 🛒 Real-world example: Type-safe shopping cart system
{
...
class ShoppingCart<TCustomer extends Customer = Customer> {
  ...
  setCustomer(customer: TCustomer): void {
    ...
    console.log(\`🛒 Cart assigned to \${customer.name} (\${customer.membershipLevel} member)\`);
  }
  ...
  removeItem(productId: string): boolean {
    ...
    if (index !== -1) {
      ...
      console.log(\`🗑️ Removed \${removedItem.product.name} from cart\`);
      return true;
    }
  ...
  getSummary(): string {
    ...
    let summary = \`🛒 Cart Summary (\${itemCount} items):\\n\`;
    
    this.items.forEach(item => {
      const itemTotal = item.product.price * item.quantity;
      summary += \`  • \${item.quantity}x \${item.product.name} - $\${itemTotal.toFixed(2)}\\n\`;
    });
